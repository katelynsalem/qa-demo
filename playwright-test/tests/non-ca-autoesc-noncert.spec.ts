...
/*
--
-- Command: npx playwright test tests/txrequests/non-ca-autoesc-noncert.spec.ts
--
-- Test List:
-- 
...
test.describe('Non-CA Claim autoescalated to non-certified workflow', () => {

    test('Create a new RFA that will auto-escalate and be non-certified by an adjuster', async ({ page }) => {
        const loginPage = new LoginPage(page);
        ...
        await test.step('Manually non-certify request', async () => {
            let determination = "Non-Certify";
            await referral.selectDetermination(determination);
            await referral.clickComplete();
        })

        await test.step('Enter Non-Certified Determination information', async () => {
            let noncertReason = "Claim Denied";
            await determination.noncertDeterminationPage(noncertReason);
            await determination.clickNext();
        })

        await test.step('Verify no vendor selection needed for requests', async () => {
        ...
        await test.step('Verify non-certified letter was generated', async () => {
            const fileName = "Not Certified";
            await doclettertab.downloadDoc(fileName);
        })
        
        await test.step('Verify no vendor information displays in MedAuth', async () => {
